...
#[test]
fn mismatched_callback_path_is_rejected() {
    assert_eq!(
        validate_redirect_uri("https://example.test/wrong".to_owned()),
        Err(StatusCode::INTERNAL_SERVER_ERROR)
    );
}
...
fn redirect_uri_with_fragment_is_rejected() {
    assert_eq!(
        validate_redirect_uri("https://example.test/auth/callback#fragment".to_owned()),
        Err(StatusCode::INTERNAL_SERVER_ERROR)
    );
}
